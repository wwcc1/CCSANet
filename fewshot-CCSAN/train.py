...
def train(epoch, model, criterion, optimizer, trainloader, learning_rate, use_gpu):
    ...
    for batch_idx, (images_train, labels_train, pidp, images_test, labels_test, pids) in enumerate(trainloader):
        ...
        ytest, ytrain, cls_scores, cls_scores1, cls_scores2,dd = model(images_train, images_test, labels_train_1hot,
                                                                    labels_test_1hot)
        loss1 = criterion(ytest, pids.view(-1))##对查询集全连接分类的结果
        loss2 = criterion(ytrain, pidp.view(-1))##对原型全连接分类的结果
        loss6 =criterion(dd, pids.view(-1))
        # loss7 = criterion(aa, pidp.view(-1))
        loss3 = criterion(cls_scores, labels_test.view(-1))#原型小样本分类结果
        loss4 = criterion(cls_scores1, labels_train.view(-1))
        loss5 = criterion(cls_scores2, labels_train.view(-1))
        loss = loss1 + loss2 + 0.5 * loss3 + 0.1 * loss4 + 0.05 * loss5+loss6*0.6 #80.52
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
